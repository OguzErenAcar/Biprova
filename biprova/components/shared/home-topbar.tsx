...
function MobileDrawer({ open, onClose, projects }: { open: boolean; onClose: () => void; projects: DrawerProject[] }) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="relative flex flex-col items-center px-5 py-5 border-b border-slate-100 gap-2">
          <button
            onClick={onClose}
            className="absolute right-5 top-5 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors"
            aria-label="Kapat"
          >
