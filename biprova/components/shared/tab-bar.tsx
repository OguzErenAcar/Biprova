...
function TabItem({ tab, active, onNavigate }: { tab: Tab; active: boolean; onNavigate: () => void }) {
  ...
  return (
    <Link
      href={tab.href}
      onClick={onNavigate}
      className={`flex flex-col items-center px-3 h-[60px]  flex-1 no-underline tab-inactive`}
      onMouseEnter={() => lottieRef.current?.play()}
      onMouseLeave={() => lottieRef.current?.stop()}
    >
      ...
      <span className={`text-[10px] mt-0.5 font-semibold whitespace-nowrap transition-opacity ${active ? "text-black opacity-100" : "text-white opacity-70"}`}>
        {tab.label}
      </span>
    </Link>
  );
