...
function NavItem({
  ...
  return (
    <Link
      href={href}
      className={`flex text-label items-center gap-[0.65rem] px-3 py-[0.65rem] rounded-[10px] text-body font-semibold mb-0.5 transition-all duration-150 no-underline ${
        isActive
          ? "bg-black text-white"
    ...
    >
      <span className="w-6 h-6 shrink-0">
        <Lottie
          ...
          loop={false}
          autoplay={false}
          style={{ width: 27, height: 27, filter: "brightness(0) invert(1)" }}
        />
      </span>
      {label}
    </Link>
...
export function Sidebar({ projects = [], showInfoDialog = false, lastActiveUsers }: SidebarProps) {
  const pathname = usePathname();
  const [savedOpen, setSavedOpen] = useState(true);
  const [infoOpen, setInfoOpen] = useState(showInfoDialog);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <svg
          className={`w-3 h-3 text-white/50 transition-transform duration-200 ${savedOpen ? "rotate-180" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2.5}
