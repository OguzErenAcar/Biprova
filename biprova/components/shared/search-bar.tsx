...
export function SearchBar({ className }: { className?: string }) {
  ...
  return (
    <div ref={containerRef} className={`relative flex-1 min-w-0 max-w-[200px] ${className ?? ""}`}>
      ...
      <input
        ...
        className="w-full bg-surface border-[1.5px] border-black rounded-[10px] pl-9 pr-4 py-[0.6rem] font-body text-xs text-slate-900 outline-none focus:border-blue-600 placeholder:text-slate-400 transition-colors duration-200"
      />

      {open && (
        <div className="absolute left-0 top-full z-50 pt-2 w-full">
          <div className="bg-surface border-[1.5px] border-slate-200 rounded-2xl shadow-xl overflow-hidden">
            {isPending ? (
