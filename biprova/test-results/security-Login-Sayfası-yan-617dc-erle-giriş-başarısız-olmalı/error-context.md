...
Locator: locator('text=/hatalı|geçersiz|yanlış/i')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=/hatalı|geçersiz|yanlış/i')
...
- generic [active] [ref=e1]:
  - main [ref=e3]:
    - generic [ref=e4]:
      ...
      - generic [ref=e7]:
        - generic [ref=e8]:
          ...
          - textbox "E-posta" [ref=e10]:
            ...
            - text: yanliskullanici@test.com
        - generic [ref=e11]:
          - generic [ref=e12]: Şifre
          - textbox "Şifre" [ref=e13]:
            - /placeholder: ••••••••
            - text: YanlisParola123!
        - paragraph [ref=e14]: Hesabınız çok fazla başarısız deneme nedeniyle 15 dakika kilitlendi.
      ...
      - paragraph [ref=e18]:
        - text: Hesabın yok mu?
        - link "Kayıt ol" [ref=e19] [cursor=pointer]:
          - /url: /signup
      - link "← Ana sayfaya dön" [ref=e20] [cursor=pointer]:
        - /url: /
...
```ts
  ...
  53  |     // Validation hataları görünmeli — hata mesajı paragrafını hedefle
  54  |     await expect(page.locator('p.text-red-500').first()).toBeVisible({ timeout: 3000 })
  55  |   })
  56  | 
...
> 62  |     await expect(page.locator('text=/hatalı|geçersiz|yanlış/i')).toBeVisible({ timeout: 5000 })
  ...
  63  |   })
  64  | 
  65  |   test('giriş yapmış kullanıcı login sayfasına gelince dashboard\'a yönlenmeli', async () => {
  ...
  129 |   test('404 sayfası stack trace içermemeli', async ({ page }) => {
  130 |     await page.goto(`${BASE_URL}/bu-sayfa-yok-${Date.now()}`)
  131 |     const content = await page.content()
  132 |     // Gerçek stack trace satırlarını yakala — script src path'lerini değil
  133 |     expect(content).not.toMatch(/at Object\.<anonymous>|at Module\._compile|\.ts:\d+:\d+/)
  134 |   })
  135 | 
  136 |   test('dashboard 401/403 hatasında sunucu detayı sızmamalı', async ({ page }) => {
  137 |     await page.goto(`${BASE_URL}/dashboard/projects/00000000-0000-0000-0000-000000000000`)
  138 |     const content = await page.content()
  139 |     expect(content).not.toMatch(/SUPABASE|service_role|eyJhbGci/)
  140 |   })
  141 | })
  142 | 
```