...
import { deleteProject, getSkills, leaveProject, transferProjectCreator } from '@/features/projects/actions';
...
export function PanelEkip({ project }: Props) {
  ...
  return (
    <div id="panel-ekip" className="flex flex-col gap-[1.2rem]">
      ...
      {viewer.is_creator && (
        <TransferCreatorSection projectId={project.id} members={project.members} viewerId={viewer.id} />
      )}
    </div>
  );
}
