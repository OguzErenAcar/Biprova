...
export function ProjectTabView({ project }: Props) {
  ...
  const TABS: {
    ...
    { key: 'chat',       label: 'Chat',       icon: MessageCircle, requiresTeam: true,  count: unreadCount },
  ...
  return (
    <>
      ...
      <div
        id="project-tabs"
        className="sticky top-0 z-30 backdrop-blur-[12px] border-b"
      >
      ...
      {/* Content */}
      <div ref={contentRef} className={`lg:pb-6 ${activeTab === 'chat' ? 'pb-0' : 'p-4 sm:p-6 pb-[calc(1rem+56px)]'}`}>
        {activeTab === 'genel' && (
          <PanelGenel
            project={project}
        ...
        {activeTab === 'chat' && project.team_id ? (
          <PanelChat
            ...
            viewerAvatar={project.viewer.avatar_url}
          />
        ) : activeTab === 'chat' ? (
