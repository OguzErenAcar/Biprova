            ...
            {modal.type === 'leave' && modalError && team.viewer.in_founding_project && (() => {
              ...
              return (
                <div className="mb-4">
                  ...
                  <button
                    onClick={() => { setModal(null); setModalError(''); router.push(`/dashboard/projects/${fp.id}`); }}
                    className="flex items-center justify-between w-full px-3 py-2 rounded-[8px] border-[1.5px] border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition-colors text-left group"
                  >
                    <div>
                      <span className="text-[0.83rem] font-bold text-slate-700 group-hover:text-blue-600 block">{fp.title}</span>
                      {fp.leader_id === team.viewer.id && (
